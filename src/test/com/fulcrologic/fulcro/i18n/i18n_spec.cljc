...
(specification "Base translation -- tr"
  (assertions
    ...
    (tr 4) =fn=> (fn [s] (re-matches #".*on line [1-9][0-9]* in com.*" s))))
...
(specification "Message translations with context"
  (assertions
    ...
    (trc 1 4) =fn=> (fn [s] (re-matches #".*on line [1-9][0-9]* in com.*" s))
  ...
  (i18n/with-locale deflt-format locale-with-no-translations
    ...
    (let [s "str"]
      (assertions
        ...
        (trf s) =fn=> (fn [s] (re-matches #".*on line [1-9][0-9]* in com.*" s))))
    (assertions
...
#?(:clj
   (specification "Locale loading from PO files."
     (let [xlation         (i18n/load-locale "resources" :es)
...
#?(:clj
   (specification "Locale override in SSR"
     (let [initial-tree     (comp/get-initial-state Root {})
           es-locale        (i18n/load-locale "resources" :es)
